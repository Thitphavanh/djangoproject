...
> ใช้คู่กับ [Django-Shell-Query-Tutorial.md](Django-Shell-Query-Tutorial.md) (บทที่ 0–13)
> อยากรู้เทคนิคขั้นสูงต่อ (Many-to-Many, prefetch_related, bulk_create, Case/When, Subquery, transaction, signal ฯลฯ) ไปต่อที่ [Django-Shell-Query-Tutorial-Advanced.md](Django-Shell-Query-Tutorial-Advanced.md)

---

## สารบัญ

1. [ผ่าไฟล์ models.py ทีละบรรทัด](#ส่วนที่-1) (รวมโมเดล `Tag` และ `ManyToManyField` ที่ 1.5)
...
## 1.5 โมเดล `Tag` และฟิลด์ `ManyToManyField` — ความสัมพันธ์แบบ N:N
...
โครงสร้างเหมือน `Brand`/`Category` ทุกอย่าง (คลาสธรรมดาที่สืบทอดจาก `models.Model`, มี `__str__`) — จุดที่ต่างและสำคัญคือบรรทัดในคลาส `Product`:
...
| ส่วน                        | อธิบาย                                                                                                                                                                                                                                                                        |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tags`                        | ชื่อฟิลด์ — แต่**ไม่ได้สร้างคอลัมน์ `tags_id` ในตาราง `shop_product`** เหมือนที่ `ForeignKey` ทำ                                                                                                                       |
| `models.ManyToManyField(...)` | ประเภทฟิลด์แบบ "หลายต่อหลาย" — สินค้า 1 ตัวมีได้หลายแท็ก และแท็ก 1 แท็กก็อยู่กับสินค้าได้หลายตัว                                                                                                |
| `Tag` (arg ตัวแรก)      | ปลายทางที่เชื่อมไป เหมือนอาร์กิวเมนต์แรกของ`ForeignKey`                                                                                                                                                                                 |
| `related_name='products'`     | ทางเดินย้อนกลับจาก Tag → Product เขียน`tag.products.all()` ได้ (เหมือนหลักการเดียวกับ FK ในบทที่ 1.4)                                                                                                                      |
| `blank=True`                  | สินค้าจะไม่มีแท็กเลยก็ได้ (ไม่บังคับต้องมีอย่างน้อย 1 แท็ก)                                                                                                                                                                    |
| ไม่มี`on_delete`         | เพราะ M2M ไม่ใช่ FK — ไม่มีทิศทาง "แม่-ลูก" ตายตัวแบบเดียวกัน ถ้าลบ Tag ทิ้ง Django จะลบแค่**แถวคู่ในตารางกลาง**ที่เชื่อมกับแท็กนั้น ไม่กระทบตัว Product เลย |

### เบื้องหลัง: Django สร้างตารางที่ 4 ให้แบบไม่ต้องเขียนเอง
...
CREATE TABLE shop_product_tags (              -- ตารางกลาง (join table) Django สร้างให้อัตโนมัติ
    id         integer PRIMARY KEY AUTOINCREMENT,
    product_id integer NOT NULL REFERENCES shop_product(id),
    tag_id     integer NOT NULL REFERENCES shop_tag(id),
    UNIQUE (product_id, tag_id)                -- คู่ product-tag ซ้ำกันไม่ได้
);
```

ไม่มีคอลัมน์ `tags` อยู่ในตาราง `shop_product` เลย — ข้อมูลความสัมพันธ์ทั้งหมดอยู่ที่ตาราง `shop_product_tags` ต่างหาก นี่คือความต่างหลักจาก `ForeignKey` ที่ยัดคอลัมน์ `_id` ไว้ในตารางตัวเองตรง ๆ

### ทำไมแก้แท็กด้วย `.save()` ไม่ได้ ต้องใช้ `.add()`/`.set()`/`.remove()`/`.clear()`

เพราะ `product.tags` ไม่ใช่ค่าธรรมดาที่เก็บอยู่ใน object เดียวกัน (ต่างจาก `product.price`) — มันคือ Manager ตัวหนึ่งที่ชี้ไปตารางกลาง `shop_product_tags` ทุกเมธอด (`.add()`, `.remove()`, `.set()`, `.clear()`) จะยิง SQL ไปแก้ตารางกลางนั้น**ทันที** ไม่ต้องรอ `.save()` ของ Product เลย

> 📖 ตัวอย่างการใช้งานจริงแบบ step-by-step (`.add()` → `.remove()` → `.set()` → `.clear()`) และกับดักเรื่อง `annotate(Count())` นับซ้ำตอน join ผ่าน M2M อ่านต่อได้ที่ [Django-Shell-Query-Tutorial-Advanced.md](Django-Shell-Query-Tutorial-Advanced.md) บทที่ 14
